...
export class RecipesService {
  ...
  async removeRatingByRecipeID(recipeId: number) {
    await this.checkCanUpdate(recipeId);
    const record = await this.repositoryRating.delete({
      recipe: { id: recipeId },
    });
    if (!record?.affected) {
      throw new RecordNotFoundException();
  ...
  async removeImageByRecipeID(recipeId: number) {
    await this.checkCanUpdate(recipeId);
    const record = await this.repositoryImage.delete({
      recipe: { id: recipeId },
    });
    if (!record?.affected) {
      throw new RecordNotFoundException();
